...
* **`confetti/email/statuses=[invalid, not_in_client, tracker_error, unhandled_error, affected]`**
    * Типы сообщений о результатах обновления, которые необходимо отправить (в порядке отправки кроме `affected`, который всегда идет первым). Доступны следующие варианты:

        | Статус | Описание |
        |--------|----------|
        | `invalid` | Торренты с некорректным форматом данных (битые файлы) |
        ...
        | `unhandled_error` | Непредвиденная ошибка при обработке торрента. Рекомендуется почитать результат работы [emupdate](emupdate), чтобы узнать, какое произошло исключение |
...
* **`confetti/telegram/statuses=[invalid, not_in_client, tracker_error, unhandled_error, affected]`**
    * Типы сообщений о результатах обновления, которые необходимо отправить (в порядке отправки). Доступны следующие варианты:
...
После настройки всех оповещений вы можете протестировать их с помощью команды [emconfetti-demo](emconfetti-demo). Она отсылает демо-сообщения всеми способами, указанными в секции `confetti`.
